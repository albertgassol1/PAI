...
def log_posterior_probs(x):
    ...
    # Log likelihood
    log_p_x_Hi = np.array([np.sum(elem.logpdf(x)) for elem in HYPOTHESIS_SPACE])

    # Log of sum in Bayes rule
    log_sum = logsumexp(np.log(PRIOR_PROBS) + np.array([np.sum(elem.logpdf(x)) for elem in HYPOTHESIS_SPACE]))   

    # Add all terms
    log_p = np.log(PRIOR_PROBS) + log_p_x_Hi - log_sum*np.ones(3)
