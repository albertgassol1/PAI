...
class MLPActorCritic(nn.Module):
    ...
    def step(self, state):
        ...
        with torch.no_grad():
            # Get policy distribution from the NN and sample it to get an action given the current state
            pi_dist, log_prob_pi = self.pi.forward(state)
            action = pi_dist.sample()
            # Compute log_ppi_lossrob of the sampled action using the policy distribution
            log_prob_action = pi_dist.log_prob(action)
            # Compute value function at current state using estimate from NN
            value_function = self.v.forward(state)

        return action.item(), value_function.item(), log_prob_action.item()
...
class VPGBuffer:
    ...
    def end_traj(self, last_val=0):
        ...
        deltas = rews[:-1] + discount_cumsum(vals[1:] , self.gamma) - vals[:-1]
...
class Agent:
    ...
    def pi_update(self, data):
        ...
        # Get policy distribution and its log(pi(a|s)), used in the policy gradient
        pi_dist, log_prob_pi = self.ac.pi.forward(obs, act)

        # Define loss (must be negative as we will do gradient ascent and optimizers work with descent)
        loss = 0
        for i in range(len(ret)):
            # We use reward-to-go for the policy gradient
            loss -= log_prob_pi[i] * ret[i]
    ...
    def v_update(self, data):
        ...
        # Iterate to update value function
        for i in range(self.num_it_v_update):
            loss = 0
            self.v_optimizer.zero_grad()
            vals = self.ac.v.forward(obs)
            deltas = ret[:-1] + discount_cumsum(vals[1:].detach().numpy(), 0.99) - vals[:-1]
            phi = discount_cumsum(deltas.detach().numpy(), 0.99*0.97)
            # Estimate advantage function
            for j in range(len(phi)):
                loss -= phi[j]*log_prob_pi[i]
            loss = loss
            if i < self.num_it_v_update - 1:
